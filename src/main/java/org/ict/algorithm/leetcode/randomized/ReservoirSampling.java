 ...
 *   The probability that the last item is in final reservoir = The probability
 *   that one of the first k indexes is picked for last item = k/n
 ...
 *   that one of the first k indexes is picked in iteration for stream[n-2]] X [Probability
 *   that the index picked in iteration for stream[n-1] is not same as index picked for stream[n-2] ] = [k/(n-1)]*[(n-1)/n] = k/n.
 ...
 *   The probability that an item from stream[0...k-1] is
 *   in final array = Probability that the item is not picked when items stream[k], stream[k+1], …. stream[n-1]
