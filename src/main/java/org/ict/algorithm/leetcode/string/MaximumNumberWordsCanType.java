 ...
 * LC1935
 */
public class MaximumNumberWordsCanType {
    ...
    public int canBeTypedWords(String text, String brokenLetters) {
        ...
        for (String word : arr) {
            if (!checkWord(word, brokenLetters)) {
                continue;
            }
            res++;
        }
    ...
    public boolean checkWord(String word, String brokenLetters) {
        for (char ch : word.toCharArray()) {
            if (brokenLetters.contains(Character.toString(ch))) {
                return false;
            }
        }
        return true;
