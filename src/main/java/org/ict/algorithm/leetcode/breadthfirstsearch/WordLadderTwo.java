...
public class WordLadderTwo {
	...
	public static void main(String[] args) {
	    ...
	    */
		...
		/**
		 * step into dfsAddPath.
         * After add:poon, list is [toon, poon], Prepare to recursion invoke dfsAddPath
         * step into dfsAddPath.
         * After add:poin, list is [toon, poon, poin], Prepare to recursion invoke dfsAddPath
         * step into dfsAddPath.
         * After add:poie, list is [toon, poon, poin, poie], Prepare to recursion invoke dfsAddPath
         * step into dfsAddPath.
         * After add:plie, list is [toon, poon, poin, poie, plie], Prepare to recursion invoke dfsAddPath
         * step into dfsAddPath.
         * After add:plee, list is [toon, poon, poin, poie, plie, plee], Prepare to recursion invoke dfsAddPath
         * step into dfsAddPath.
         * After add:plea, list is [toon, poon, poin, poie, plie, plee, plea], Prepare to recursion invoke dfsAddPath
         * step into dfsAddPath.
         * from:plea equals to:plea
         * After recursion invoke dfsAddPath of next: plea
         * After remove last of list: [toon, poon, poin, poie, plie, plee]
         * After recursion invoke dfsAddPath of next: plee
         * After remove last of list: [toon, poon, poin, poie, plie]
         * After recursion invoke dfsAddPath of next: plie
         * After remove last of list: [toon, poon, poin, poie]
         * After recursion invoke dfsAddPath of next: poie
         * After remove last of list: [toon, poon, poin]
         * After recursion invoke dfsAddPath of next: poin
         * After remove last of list: [toon, poon]
         * After recursion invoke dfsAddPath of next: poon
         * After remove last of list: [toon]
         * [[toon, poon, poin, poie, plie, plee, plea]], time cost:8
		 */
	    ...
	    System.out.println(result + ", time cost:" + duration);
	}
	 ...
	 public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
		 ...
		 while (!curLevel.isEmpty() && !foundEndWord) {
			 // this is very important to minimize the graph size and avoid backtrack of the path
			 wordDict.removeAll(curLevel);
			 Set<String> nextLevel = new HashSet<>();
			 for (String curWord : curLevel) {
				 graph.put(curWord, new ArrayList<>());
			 ...
			 }// end of cur level search
			 curLevel = nextLevel;
		 } // end of all level search
		 
		 if (!foundEndWord) {
			 return result;
		 }
		 // 2.find the path by DFS of recursion
		
		 List<String> list = new ArrayList<>();
		 list.add(beginWord);
		 dfsAddPath(beginWord, endWord, result, graph, list);
	     return result;
	 }
	 
	 private void dfsAddPath(String from, String to, List<List<String>> result, Map<String, List<String>> graph, List<String> list) {
		 System.out.println("step into dfsAddPath.");
		 if (from.equals(to)) {
			 System.out.println("from:" + from + " equals to:" + to);
		 ...
		 for (String next : graph.get(from)) {
			 list.add(next);
			 System.out.println("After add:" + next + ", list is " + list + ", Prepare to recursion invoke dfsAddPath");
			 
			 dfsAddPath(next, to, result, graph, list);
			 System.out.println("After recursion invoke dfsAddPath of next: " + next);
			 list.remove(list.size() - 1);// why add this line?
			 System.out.println("After remove last of list: " + list);
			 //list.remove(list.size() -1) will clear the current path with node-next of current layer and return to next layer of adjacency-list
			 // If you delete this remove operation, then you will get only one path.
			 // And the list size will grow bigger and bigger.
		 }
	 }
}
