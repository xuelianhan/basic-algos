 ...
 * operations, along with methods for peeking at the minimum, testing 
...
public class MinPQ<Key> implements Iterable<Key> {
    ...
    public void insert(Key x) {
        ...
        // add x, and precolate it up to maintain heap invariant
        pq[++n] = x;
        swim(n);
        assert isMinHeap();
    }
    ...
    private boolean isMinHeap(int k) {
        ...
        // it is a min Heap. otherwise, it returns false in early steps.
        if (k > n) return true;
        int left = 2*k;
        int right = 2*k + 1;
