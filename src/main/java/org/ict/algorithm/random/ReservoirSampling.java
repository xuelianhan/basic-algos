package org.ict.algorithm.random;
 ...
 *  The solution also suits well for input in the form of stream. 
 ...
 *   For every such stream item stream[i], we pick a random index from 0 to i and if the picked index is one of the first k indexes, 
 *   we replace the element at picked index with stream[i]
 *   To simplify the proof, let us first consider the last item. 
 *   The probability that the last item is in final reservoir = The probability that one of the first k indexes is picked for last item = k/n (the probability of picking one of the k items from a list of size n)
 *   Let us now consider the second last item. 
 *   The probability that the second last item is in final reservoir[] = [Probability that one of the first k indexes is picked in iteration for stream[n-2]] X [Probability that the index picked in iteration for stream[n-1] is not same as index picked for stream[n-2] ] = [k/(n-1)]*[(n-1)/n] = k/n.
...
public class ReservoirSampling {
    ...
    public static void selectKItems(int[] stream, int n, int k) {
        ...
        System.out.println("Following ar k randomly selected items");
